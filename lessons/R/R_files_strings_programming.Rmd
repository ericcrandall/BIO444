---
title: "Introduction to R: Files, Strings, Programming"
...
```{r include=FALSE}
plankton_histogram<-function(pathname, plankter, breaks){
  #get the filenames
  filenames<-list.files(path=pathname,pattern="m.txt")
  allplankton<-NULL
  # it might be smart to initialize an empty 300x3 data frame, but we won't
  #allplankton <- data.frame( depth_m=rep(0, 300), plankter_name=rep(0,300), size_um=rep(0,300))
  for(file in filenames){
    plankton<-read.csv(file=file)
    allplankton<-rbind(allplankton,plankton)
  
  }
  depth<-allplankton[which(allplankton$plankter_name==plankter),1]
  hist(depth, breaks=breaks)
}
