...
`unique(pelagics$genus)`{rsplus}
`<code>
## [5] "Rastrelliger"
```

`unique(pelagics$decimalLatitude)`{rsplus}
`<code>
##  [8]  -8.6510   3.5915  -5.1189  -5.8192  -6.0000   1.4820  -6.0925

## [15]  -2.5330  -1.8817  -6.9667  -8.5912 -10.4461  -5.1309  -0.5897
## [22]  -9.4456   1.1255  -8.5254  -8.5073  -0.9500  -1.1422
```

```{r}
newdata<-unique(pelagics[1:21,])
newdata`<-unique(pelagics[2:21,])</code>`
### Summarizing Data into Tables

`table()` is handy for making "pivot tables"

`table(pelagics$country,pelagics$genus)`{rsplus}
`<code>
...
```
`<code>
## country            Auxis Euthynnus Katsuwonus Rastrelliger Scomberomorus
...
```{r}
##3 dimensional table!
bymarket`<-table(pelagics$occurrenceRemarks,pelagics$genus,pelagics$country)</code>`
## Subsetting (indexing)
...
Indexing with dataframe and matrices goes ''%%[row, column]%%'' So: 1. select columns by name

```{r}
myvars <- c("materialSampleID", "genus", "species")
newdata `<- pelagics[,myvars]</code>`
 1.  select columns or rows by number

```{r}
newdata <- pelagics[,c(1,2,3)]
newdata <- pelagics[,c(1,5:10)]
newdata `<- pelagics[1:10,1:10]</code>`
 1.  Dropping columns and rows

```{r}
newdata <- pelagics[,-c(1,2,3)]
newdata <- pelagics[,-c(1,5:10)]
newdata `<- pelagics[-c(1:10),-c(1:10)]</code>`
 1.  Using `which()`

`which(pelagics$country=="Papua New Guinea")`{rsplus}

`which(pelagics$country=="Papua New Guinea"| pelagics$country=="Solomon Islands")`{rsplus}
`<code>
...
newdata `<- pelagics[which(pelagics$country=="Papua New Guinea" | pelagics$country=="Solomon Islands" & (pelagics$country=="Euthynnus")),]</code>`
### Sorting vectors and dataframes

**We will also begin to see how we "pipe" together commands in R**

```{r}
student<-c("david","jess","liz l.","amelia","evan","drew","kalia")
sex<-c("m","f","f","f","f","m","f")
shoe<-c(12,8,7,6,10.5,11,7)
shoes<-as.data.frame(cbind(student,sex,shoe))
shoes[,3]`<-as.numeric(as.character(shoes[,3]))</code>`
`sort()` sorts vectors

`sort(shoes[,1])`{rsplus}
`<code>
## Levels: amelia david drew evan jess kalia liz l.
```
...
`order(shoes[,1])`{rsplus}

`order(shoes[,3])`{rsplus}

`shoes[order(shoes[,3]),]`{rsplus}
`<code>
## 4  amelia   f  6.0
...
otherway`<-shoes[order(shoes[,3], decreasing=T),]</code>`
...
shoes<-shoes[sample(1:7,replace=F),]
...
`merge(shoes,moredata,by="student")`{rsplus}
`<code>
## 1  amelia     f  6.0     f 6.75   29

## 2   david     m 12.0     m 7.50   30
## 3    drew     m 11.0     m 7.75   25

## 4    evan     f 10.5     f 8.00   31
## 5    jess     f  8.0     f 7.00   26

## 6   kalia     f  7.0     f 7.00   32
## 7  liz l.     f  7.0     f 7.25   34
```
...
Download *this* tab-delimited data frame and import it into R. Then write a single line of R code to answer each of these 3 challenges:
...
Provide a dokuwiki-formatted table that is sorted alphabetically by the number of RBIs they hit in 2008.
