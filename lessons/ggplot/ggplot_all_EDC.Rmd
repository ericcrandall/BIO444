...
* Here we read them in:
    ...
    #View(pv)
    ```
  Yuck! These data are full of annotations.
  Let's use our newfound regular expression and data cleaning powers to clean these data up.

```{r}
# remove the footnote refs in the dates
    pv$Date <- gsub("\\[[0-9]\\]", "", pv$Date) 
# remove the "m" and other stuff from the heights
    pv$Record <- as.numeric(gsub(" m.*", "", pv$Record))  
# convert dates to lubridate 
    pv$Date <- mdy(pv$Date)  
# change "Record" column to "Meters"
    names(pv)[names(pv) == "Record"] = "Meters" 
    ```
* Here are the first few rows of our clean data fram:
    ```{r, eval=F}
    head(pv)
    ```
...
2. Add a transparent turquoise area along the back:
    ...
    Wow! Transparency was never so easy in R base graphics!
