...
Let's look at this a little more closely.  What we are saying is that there is a certain probability, over some time interval t, that an event (coalescence) occurs, which, we've seen is 1/2N.  Now for the next interval t, the same probability exists, only this now a sample size of (1-1/(2N)).  This would continue on out as t -> infinity.  What this is is a geometric distribution: the distribution of the number of bernoulli trials (coin flips) needed to get one success.  We can summarize it as follows:
