...
We can easily create a general model of selection thusly:
...
Where mean fitness can be calculated as

$$\bar w = p^2w_{AA} + 2pqw_{Aa} + q^2w_{aa}$$

or we can also calculate &Delta;p like so:
...
```
Now, what if we had weaker recessive selection, say with s=.1?
```{r,eval=FALSE}
plot(1-deltap(pinit=.1,h=0,s=.1,plot=FALSE), col="blue")
```
...
```{r,eval=FALSE}
lines(1-deltap(pinit=.1,h=.5,s=.1),col="purple")
```
...
```{r,results="hide"}
x <-deltap(pinit=.1,h=0,s=1, plot=TRUE)
```
...
```{r,results="hide"}
deltap(pinit=.1,h=0,s=.1,plot=TRUE)
```
...
```{r,results="hide"}
deltap(pinit=.1,h=.5,s=.1,plot=TRUE)
```
...
All of the above have been examples of directional selection, with one allele or the other ultimately going to fixation.  But what about the case of heterozygote advantage?  The classical example of this is malaria and sickle-cell anemia, where heterozygotes for the diseased allele have resistance to malaria infection.

For example, suppose that w have the following:
...
```{r,results="hide"}
deltap(pinit=.1,h=4,s=-.16,plot=TRUE)
abline(h=0,col="red")
```
